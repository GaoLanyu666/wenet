...
from wenet.transformer.decoder_layer import DecoderLayer, SpikeDecoderLayer, SpikeTransformerDecoderLayer
...
class SpikeformerDecoder(torch.nn.Module):
    """Base class of Transfomer decoder module.
    Args:
        vocab_size: output dim
        encoder_output_size: dimension of attention
    ...
    def __init__(
        ...
        self.decoders = torch.nn.ModuleList([
            SpikeDecoderLayer(
                attention_dim,
                WENET_ATTENTION_CLASSES["spike_selfattn"](
                    attention_heads, attention_dim,
                    self_attention_dropout_rate, query_bias, key_bias,
                    value_bias, use_sdpa, n_kv_head, head_dim, node, T),
                WENET_ATTENTION_CLASSES["spike_crossattn"](
                    attention_heads, attention_dim, src_attention_dropout_rate,
                    query_bias, key_bias, value_bias, use_sdpa, n_kv_head,
                    head_dim, node, T) if src_attention else None,
                mlp_class(attention_dim,
                          linear_units,
                          dropout_rate,
                          activation,
    ...
    def forward(
        ...
        x_shape = x.size()
        tgt_mask_shape = tgt_mask.size()
        x = x.unsqueeze(0).expand(torch.Size([self.T]) + x_shape)
        tgt_mask = tgt_mask.unsqueeze(0).expand(
            torch.Size([self.T]) + tgt_mask_shape)

        memory = memory.unsqueeze(0).repeat(self.T, 1, 1, 1)  # SNN
        memory_mask = memory_mask.unsqueeze(0).repeat(self.T, 1, 1, 1)  # SNN
    ...
    def forward_one_step(
        ...
        memory_shape = memory.size()
        memory_mask_shape = memory_mask.size()
        memory = memory.unsqueeze(0).expand(
            torch.Size([self.T]) + memory_shape)
        memory_mask = memory_mask.unsqueeze(0).expand(
            torch.Size([self.T]) + memory_mask_shape)
        # print(tgt.shape)
        # print(x.shape)
    ...
    def tie_or_clone_weights(self, jit_mode: bool = True):  # 没用到
        """Tie or clone module weights (between word_emb and output_layer)
            depending of whether we are using TorchScript or not"""
        rank = int(os.environ.get('RANK', 0))
        if not self.use_output_layer:
...
class SpikeTransformerDecoder(torch.nn.Module):
    """Base class of Transfomer decoder module.
    Args:
        vocab_size: output dim
        encoder_output_size: dimension of attention
    ...
    def __init__(
        ...
        mlp_class = WENET_MLP_CLASSES[mlp_type]
        self.decoders = torch.nn.ModuleList([
            SpikeTransformerDecoderLayer(
                attention_dim,
                WENET_ATTENTION_CLASSES["spike_ln_selfattn"](
                    attention_heads, attention_dim,
                    self_attention_dropout_rate, query_bias, key_bias,
                    value_bias, use_sdpa, n_kv_head, head_dim, node, T),
                WENET_ATTENTION_CLASSES["spike_crossattn_ln"](
                    attention_heads, attention_dim, src_attention_dropout_rate,
                    query_bias, key_bias, value_bias, use_sdpa, n_kv_head,
                    head_dim, node, T) if src_attention else None,
                mlp_class(attention_dim,
    ...
    def forward(
        ...
        x = x.reshape(self.T,
                      x.size(0) // self.T, x.size(1), x.size(2)).contiguous()
        tgt_mask = tgt_mask.reshape(self.T,
                                    tgt_mask.size(0) // self.T,
                                    tgt_mask.size(1),
                                    tgt_mask.size(2)).contiguous()
        x = x.mean(0)  # SNN
    ...
    def forward_one_step(
        ...
        y_shape = torch.Size([self.T, y.size(0) // self.T]) + y.size()[1:]

        y = y.view(y_shape).contiguous()
        y = y.mean(0)  # SNN
        if self.use_output_layer:
            y = torch.log_softmax(self.output_layer(y), dim=-1)
        return y
