...
class SSA_ConvBN(nn.Module):
    ...
    def _forward_linearx(self, name: str, x: torch.Tensor) -> torch.Tensor:
        ...
        if name == 'query':
            ...
            x = self.q_bn(x).transpose(1, 2).contiguous()
            x = self.q_lif(
            ...
            x = self.k_bn(x).transpose(1, 2).contiguous()
            x = self.k_lif(
                x.reshape(self.T, int(x.size(0) / self.T), x.size(1),
                          x.size(2)).contiguous())  #
            x = x.flatten(0, 1)
            ...
            x = self.v_bn(x).transpose(1, 2).contiguous()
            x = self.v_lif(
                x.reshape(self.T, int(x.size(0) / self.T), x.size(1),
                          x.size(2)).contiguous())  #
            x = x.flatten(0, 1)
    ...
    def forward_attention(
        ...
        # x = x.flatten(0, 1)
        x = self.out_bn(x).transpose(1, 2).contiguous()
        x = x.view(x_shape)  # (batch, ..., time1, d_model)
        # print("out",x.shape)
        x = self.out_lif(
            x.reshape(self.T, int(x.size(0) / self.T), x.size(1),
...
class RelPositionSSA_ConvBN(SSA_ConvBN):
    ...
    def __init__(self,
        ...
        self.linear_pos = nn.Conv1d(n_feat,
                                    n_feat,
                                    kernel_size=1,
                                    stride=1,
                                    bias=False)
    ...
    def forward(
        ...
        p = self.linear_pos(pos_emb.transpose(1, 2).contiguous()).transpose(
            1, 2).contiguous()
...
class SSA_LN(nn.Module):
    ...
    def forward_attention(
        ...
        # print("lif",x.shape)
        x = self.attn_lif(x.view(x_lif_shape).contiguous())
        x = x.flatten(0, 1)
        x = self.linear_out(x)
        # print("attention",x.shape)
        # if len(x.shape) >= 4:  # SNN search
        ...
        x = x.view(x_shape)  # (batch, ..., time1, d_model)
        # print("out",x.shape)
        x = self.out_lif(x.view(x_lif_shape).contiguous()).flatten(0, 1)
        return x  # (Tbatch, ...,  time1, d_model)
