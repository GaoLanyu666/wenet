...
def img_featuremap_visual(
    ...
    img = torchvision.utils.make_grid(feature,
                                      nrow=nrow,
                                      padding=padding,
                                      pad_value=pad_value)
    img = img.detach().cpu()
    ...
    title = str('hwc-') + str(h) + '-' + str(w) + '-' + str(
        c) if feature_title is None else str(feature_title)
    ...
    if save_feature:
        ...
        out_root = title + '.jpg' if out_dir == '' or out_dir is None else os.path.join(
            out_dir, title + '.jpg')
...
def audio_featuremap_visual(
    ...
    # title = str(images.shape) if feature_title is None else str(feature_title)
    title = str('tdb-') + str(t) + '-' + str(d) + '-' + str(
        b) if feature_title is None else str(feature_title)
    ...
    with torch.cuda.amp.autocast(enabled=True,
        ...
        with torch.no_grad():
            for batch_idx, batch in enumerate(test_data_loader):
                ...
                audio_featuremap_visual(
                    feature=encoder_out,
                    out_dir=args.result_dir,  # 特征图保存路径文件
                    feature_title=batch_idx,  # 特征图名字，默认以shape作为title
                )
