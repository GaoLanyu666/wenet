...
int getUserOpts(int ac, char** av) {
  ...
  while ((i = getopt(ac, av, "daupf:c:t:s:h:")) != EOF) switch (i) {
      case 'd':
        debug = 1;
        break;
   ...
   */
  nfn = ac - optind;
  if ((nfn + 1) / 2 != 1 || outheader == NULL || chanout < 0) return 1;
  ...
  outorder = (!strcmp(outheader, "AU") || !strcmp(outheader, "AIF")) ? "10"
             : (!strcmp(outheader, "RIFF"))                          ? "01"
                                            : nativorder;

  /* Output sample coding will be PCM for aif files (aif does not support ULAW)
   */
  if (!strcmp(outheader, "AIF")) typeout = PCM;

  if (debug) {
    fprintf(stderr,
            "command-line params: sizeout=%d, typeout=%d, outorder=%s, "
            "outheader=%s,\n",
            sizeout, typeout, outorder, outheader);
    fprintf(stderr,
            "  bgnsamp=%d, bgnsec=%f, endsamp=%d, endsec=%f, chanout=%d\n",
...
int doConversion(char* inpname, char* outname) {
  ...
  endout = (endsamp > sampcount || endsec >= totalsec || endsamp + endsec == 0)
               ? sampcount
           : (endsamp > 0) ? endsamp
                           : (int)(endsec * samprate);
  /*
  ...
  if (endout < 0 || endout > sampcount) endout = sampcount;
  if (startout > 0 || endout < sampcount) sampcount = endout - startout;

  sizeout = typeout & 3; /* yields 1 for ulaw/alaw, 2 for pcm */

  if (debug) {
    fprintf(stderr,
            "control params:  sizeout=%d, typeout=%d, outorder=%s, "
            "outheader=%s, chanout=%d,\n",
            sizeout, typeout, outorder, outheader, chanout);
    fprintf(stderr,
            "  bgnsamp=%d, bgnsec=%f, endsamp=%d, endsec=%f, startout=%d, "
            "endout=%d, sampcount=%d\n",
  ...
  if (ret) fprintf(stderr, "conversion failed for %s\n", inpname);
...
uchar pcm2alaw(short int pcmval) {
  ...
  if (seg == 8)
    ...
    aval |=
        (seg < 2) ? (pcmval >> 1) & QUANT_MASK : (pcmval >> seg) & QUANT_MASK;
    return (aval ^ mask);
  }
}
...
int copySamples(void) {
  ...
  if (startout > 0) fseek(fpin, startout * sampsize * chancount, SEEK_CUR);
  ...
  while ((nb = fread(inpbuf, 1, STD_BUF_SIZE, fpin)) > 0 &&
    ...
    if (sampsize < sizeout) { /* convert ulaw or alaw to pcm */
      ...
      for (i = 0; i < nb; i++) *sptr++ = cnvptr[*cptr++];
      nb *= 2;
      ...
      for (i = 0; i < nb; i += 2) *cptr++ = (*pcm2xptr)(*sptr++);
      nb /= 2;
