...
long uvar_get(int nbin) {
  ...
  while (nbin != 0) {
    if (nbitget >= nbin) {
      result =
          (result << nbin) | ((gbuffer >> (nbitget - nbin)) & masktab[nbin]);
      nbitget -= nbin;
      nbin = 0;
    } else {
...
long** long2d(ulong n0, ulong n1) {
  ...
  if ((array0 = (long**)malloc(n0 * sizeof(long*) + n0 * n1 * sizeof(long))) !=
      NULL) {
...
void init_offset(long** offset, int nchan, int nblock, int ftype) {
  ...
  for (chan = 0; chan < nchan; chan++)
    for (i = 0; i < nblock; i++) offset[chan][i] = mean;
}
...
void fwrite_type(long** data, int ftype, int nchan, int nitem) {
  ...
  nsampsdone += nitem;
  if (startout > nsampsdone || endout <= (nsampsdone - nitem)) return;
  ...
  /* For "sph_convert", this next part is where uncompressed samples need extra
   * (or different) handling to support output options not provided by original
   * "shorten":
   *  -- if original data was 2-channel, user may output both (dflt), or chan.1
   * or chan.2
   *  -- if output format is pcm RIFF, byte order must be LH, regardless of
   * native order
   *  -- user may force ulaw data (AU2) to pcm output, or vice-versa
   *  -- user may force ulaw or pcm data to alaw output (alaw data cannot be
   * shortened)
   *  -- user may select a time-slice from the file, skipping initial and/or
   * final portions
   */
  switch (ftype) {
    ...
    case TYPE_AU2:
      if (typeout == PCM) /* convert ulaw to pcm */
      {
        ...
        if (nchan == 1)
          for (i = 0; i < nitem; i++) *writebufp++ = ulaw2pcm[data0[i]];
        else if (nchan > chanout) /* this block (in each case) handles demux */
          for (i = 0; i < nitem; i++) *writebufp++ = ulaw2pcm[data[chanout][i]];
        else
          for (i = 0; i < nitem; i++)
            for (chan = 0; chan < nchan; chan++)
        ...
        uchar* writebufp = (uchar*)writebuf;
        if (nchan == 1)
          for (i = 0; i < nitem; i++) *writebufp++ = data0[i];
        else if (nchan > chanout)
          for (i = 0; i < nitem; i++) *writebufp++ = data[chanout][i];
        else
          for (i = 0; i < nitem; i++)
            for (chan = 0; chan < nchan; chan++) *writebufp++ = data[chan][i];
      }
      break;
    case TYPE_U8: {
      uchar* writebufp = (uchar*)writebuf;
    ...
    case TYPE_S16LH: {
      if (typeout == ULAW) { /* convert pcm to ulaw */
        ...
        if (nchan == 1)
          for (i = 0; i < nitem; i++) *writebufp++ = pcm2ulaw(data0[i]);
        else if (nchan > chanout)
          for (i = 0; i < nitem; i++) *writebufp++ = pcm2ulaw(data[chanout][i]);
        else
          for (i = 0; i < nitem; i++)
            for (chan = 0; chan < nchan; chan++)
        ...
        char* writebufp = writebuf;
        if (nchan == 1)
          for (i = 0; i < nitem; i++) *writebufp++ = pcm2alaw(data0[i]);
        else if (nchan > chanout)
          for (i = 0; i < nitem; i++) *writebufp++ = pcm2alaw(data[chanout][i]);
        else
          for (i = 0; i < nitem; i++)
            for (chan = 0; chan < nchan; chan++)
    ...
    case TYPE_ULAW: {
      uchar* writebufp = (uchar*)writebuf;
      if (nchan == 1)
        for (i = 0; i < nitem; i++) *writebufp++ = pcm2ulaw(data0[i] << 3);
      else if (nchan > chanout)
        for (i = 0; i < nitem; i++) *writebufp++ = data[chanout][i];
      else
        for (i = 0; i < nitem; i++)
...
void fix_bitshift(long* buffer, int nitem, int bitshift, int ftype) {
  ...
  if (ftype == TYPE_AU1)
    ...
    for (i = 0; i < nitem; i++) buffer[i] <<= bitshift;
}
...
int shortenXtract(void) {
  ...
  while (version > MAX_VERSION) {
    ...
    if (magic[nscan] != '\0' && byte == magic[nscan])
      nscan++;
    else if (magic[nscan] == '\0' && (byte == 1 || byte == 2))
      version = byte;
    else {
  ...
  if (version > 1) lpcqoffset = V2LPCQOFFSET;

  init_offset(offset, nchan, MAX(1, nmean), ftype);
  if (badinput) return 1;

  /* get commands from file and execute them */
  chan = 0;
  while ((cmd = uvar_get(FNSIZE)) != FN_QUIT) switch (cmd) {
      case FN_ZERO:
      case FN_DIFF0:
      case FN_DIFF1:
      case FN_DIFF2:
      ...
      case FN_QLPC: {
        ...
        if (cmd != FN_ZERO) resn = uvar_get(ENERGYSIZE);
        ...
        switch (cmd) {
          case FN_ZERO:
            for (i = 0; i < blocksize; i++) cbuffer[i] = 0;
            break;
          case FN_DIFF0:
          ...
          case FN_DIFF2:
            for (i = 0; i < blocksize; i++)
              cbuffer[i] =
                  var_get(resn) + (2 * cbuffer[i - 1] - cbuffer[i - 2]);
            break;
          case FN_DIFF3:
            for (i = 0; i < blocksize; i++)
              cbuffer[i] = var_get(resn) +
                           3 * (cbuffer[i - 1] - cbuffer[i - 2]) +
                           cbuffer[i - 3];
            break;
          case FN_QLPC:
            nlpc = uvar_get(LPCQSIZE);

            for (i = 0; i < nlpc; i++) qlpc[i] = var_get(LPCQUANT);
            for (i = 0; i < nlpc; i++) cbuffer[i - nlpc] -= coffset;
            for (i = 0; i < blocksize; i++) {
              long sum = lpcqoffset;

              for (j = 0; j < nlpc; j++) sum += qlpc[j] * cbuffer[i - j - 1];
              cbuffer[i] = var_get(resn) + (sum >> LPCQUANT);
            }
            if (coffset != 0)
              for (i = 0; i < blocksize; i++) cbuffer[i] += coffset;
            break;
        }
        ...
        if (nmean > 0) {
          ...
          for (i = 0; i < blocksize; i++) sum += cbuffer[i];
      ...
      default:
        fprintf(stderr,
                "sanity check fails trying to decode function %d in %s\n", cmd,
                inpname);
    }
  return 0;
}
