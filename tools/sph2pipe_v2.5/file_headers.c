 ...
 * readSphHeader
 *------------------------------------------------------------
 * get relevant fields from input sphere file header; return true (1) for
 *success; return false if:
 * - input file appears not to contain a sphere header
 * - header does not contain all of these fields:
 *      channel_count, sample_count, sample_rate, sample_n_bytes,
 *sample_coding(*)
...
int readSphHeader(char* hdrfname) {
  ...
  n = fread(inpbuf, 1, 1024,
            fphd); /* nearly all sphere headers are 1024 bytes */

  if (n != 1024 || strncmp(inpbuf, "NIST_1A", 7)) return 1;

  if (sscanf(&inpbuf[8], "%d", &hdrsize) != 1) return 1;
  ...
  /* from now on, inphdrsize should represent an offset into the data file
     (fpin), to the point where actual sample data begins; in the case where the
     header was read from a separate file (hdrfname is not null), this offset is
     zero
   */
  inphdrsize = hdrsize;
  if (hdrfname != NULL) {
  ...
  while (field != NULL && strcmp(field, "end_head")) {
    if (!strncmp(field, "channel_count -i ", 17))
      ...
      sscanf(field, "%s %s %s", fldname, fldtype, fldsval);
      samptype = (!strncmp(fldsval, "ulaw", 4))   ? ULAW
                 : (!strncmp(fldsval, "alaw", 4)) ? ALAW
                 : (!strncmp(fldsval, "pcm", 3))  ? PCM
                                                  : UNKNOWN;
    }
    field = strtok(NULL, "\n");
  }
  ...
  if (!samptype && (sampsize == 2 || (inporder && strlen(inporder) == 2)))
    samptype = PCM;
   ...
   */
  if (sampsize == UNKNOWN) sampsize = samptype & 3;
...
void writeSphHeader(void) {
  ...
  while (field != NULL && strcmp(field, "end_head")) {
    ...
    else if (!strncmp(field, "sample_n_bytes -i ", 18) && sampsize != sizeout)
      flen = sprintf(ohdr, "sample_n_bytes -i %d\n", sizeout);
    else if (!strncmp(field, "sample_byte_format -s", 21) &&
             (sampsize != sizeout ||
              (sizeout == 2 && strcmp(outorder, inporder)))) {
      if (sizeout == 1) {
        field =
...
void copylong(int val, char* dest, char* intended) {
  ...
  long_order.i4 = val;
  for (; i != e; i += incr) *dest++ = long_order.ch[i];
}
...
ConvertToIeeeExtended(num, bytes) double num;
char* bytes;
{
  int sign;
  int expon;
